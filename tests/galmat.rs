...
#[rustfmt::skip]
static GALMATS_BOUNDARY_HANDCHECKED: LazyLock<[na::DMatrix<f64>; 3]> = LazyLock::new(|| [
...
#[rustfmt::skip]
static GALMATS_INTERIOR_HANDCHECKED: LazyLock<[na::DMatrix<i32>; 3]> = LazyLock::new(|| [
  na::DMatrix::from_row_slice(1, 1, &[
    2
  ]),
  na::DMatrix::from_row_slice(2, 2, &[
...
#[test]
fn feec_vs_handchecked() {
  println!("boundary");
  for (dim, handchecked) in GALMATS_BOUNDARY_HANDCHECKED.iter().enumerate().skip(1) {
    let feec = feec_galmat_boundary(dim);
    compare_galmats(&feec, handchecked)
  }
  println!("interior");
  for (dim, handchecked) in GALMATS_INTERIOR_HANDCHECKED
    .iter()
    .cloned()
    .enumerate()
    .skip(1)
  {
    let feec = feec_galmat_interior(dim, 1);
    compare_galmats(&feec, &handchecked.cast())
  }
}

#[test]
fn feec_vs_fd() {
  for nboxes_per_dim in 1..=4 {
    for dim in 1..=3 {
