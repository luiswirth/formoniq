...
| `studio`    | the visualizer                      | `Scene` (the engine↔viewer seam, carrying `Complex`/`MeshCoords`/`Cochain`), `TriangleSurface3D` (the $RR^3$ bake), reduced-grade render marks (scalar density, streamline line field), a wgpu/winit/egui renderer, native and wasm |
