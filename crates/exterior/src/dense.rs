...
use geometry::{
  coord::{manifold::AffineDiffeomorphism, CoordRef},
...
pub trait ExteriorField {
  ...
  fn at_point<'a>(&self, coord: impl Into<CoordRef<'a>>) -> ExteriorElement<Self::Variance>;
}
...
pub struct ExteriorFieldClosure<V: VarianceMarker> {
  closure: Box<dyn Fn(CoordRef) -> ExteriorElement<V>>,
  dim: Dim,
  grade: ExteriorGrade,
  variance: PhantomData<V>,
}
...
impl<F: DifferentialMultiForm> FormPushforward<F> {
  pub fn new(form: F, transform: AffineDiffeomorphism) -> Self {
    Self {
      form,
