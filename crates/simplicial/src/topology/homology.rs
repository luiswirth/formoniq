...
impl Complex {
  ...
  pub fn relative_betti_number(&self, grade: Dim) -> usize {
    let ninterior = self.nsimplices(grade) - self.boundary_simplices(grade).len();
    ninterior - self.relative_boundary_rank(grade) - self.relative_boundary_rank(grade + 1)
  }

  /// $rank$ of the relative boundary operator $diff_k: C_k (K, diff K) ->
  /// C_(k-1) (K, diff K)$: [`Self::boundary_triplets`] with the boundary
  /// simplices struck from both columns (domain) and rows (codomain).
  fn relative_boundary_rank(&self, grade: Dim) -> usize {
    if grade == 0 || grade > self.dim() {
      return 0;
    }
    let boundary_cols: HashSet<usize> = self
      .boundary_simplices(grade)
      .iter()
      .map(|idx| idx.kidx)
      .collect();
    let boundary_rows: HashSet<usize> = self
      .boundary_simplices(grade - 1)
      .iter()
      .map(|idx| idx.kidx)
      .collect();
    let triplets: Vec<_> = self
      .boundary_triplets(grade)
      .into_iter()
