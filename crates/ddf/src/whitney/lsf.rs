...
/// The Whitney local shape function $W_sigma$ of a DOF subsimplex, on the
/// reference cell.
///
...
/// are the constant [`ref_difbarys`], so an LSF depends on nothing but the
/// cell dimension and the DOF vertex set -- no coordinates, no metric. This is
/// what lets Whitney forms live on a bare Regge manifold.
#[derive(Debug, Clone)]
