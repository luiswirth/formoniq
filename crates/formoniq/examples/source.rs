...
fn main() {
  ...
  for dim in 1..=3 {
    for grade in 0..=dim {
      ...
      // The h-family is generated by Freudenthal refinement of one coarse box,
      // rather than by regenerating the grid at each resolution: the meshes are
      // then nested, the coarse Whitney space a subspace of the fine one, and the
      // path is mesh-agnostic. Refining a flat cell is exact, so no geometric
      // error enters the rate.
      //
      // Each level refines the *base* mesh $R$-fold, never the previous level
      // twofold. The two differ: Freudenthal children are not similar to their
      // parent above dimension two, so iterating the subdivision leaves the Kuhn
      // family and degrades the shape constants, while a single $R$-fold
      // refinement of a Kuhn box reproduces the generator's grid cell for cell.
