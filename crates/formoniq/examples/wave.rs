...
//!
//! Semi-discrete: the Whitney space of $k$-forms in space, the symplectic
//! leapfrog in time, run at every dimension $1 <= n <= 3$ and grade
...
//! --- potential plus kinetic --- is conserved: the leapfrog is symplectic, so
//! $E$ stays flat to a bounded $O(Delta t^2)$ oscillation with no drift, at the
//! CFL step. The table shows $E$ holding across the run; the drift column is the
//! largest relative departure over all steps.
//!
...
fn main() {
  ...
  println!("Wave u_tt = -δd u on [0,π]^n, relative (Dirichlet) BC — leapfrog.");
  ...
  for dim in 1..=3 {
    ...
    for grade in 0..=dim {
      ...
      let energies: Vec<f64> = solution
        .iter()
        .map(|s| s.energy(&laplace, &mass, dt))
        .collect();
      let e0 = energies[0];
