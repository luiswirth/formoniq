use crate::{
  ...
  Dim, RiemannianComplex, RiemannianManifold,
};
...
impl LocalComplex {
  pub fn new(subs: Vec<Vec<KSimplexIdx>>, orientation: Sign, edge_lengths: EdgeLengths) -> Self {
    let dim = subs.len() - 1;
    let metric = edge_lengths.to_regge_metric(dim);
    Self {
      subs,
      orientation,
      edge_lengths,
